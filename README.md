...
From now on, we refer to three stages of the training procedure: Stage 0 that trains both SSN and readout parameters for the general orientation discrimination task (pretraining). Stage 1, that trains the readout parameters for the fine orientation discrimination task, and Stage 2, that trains the SSN parameters for the fine orientation discrimination task. There are a few exceptions for some control configurations, e.g., when Stage 1 and 2 train the parameters for the general discrimination task. 
...
Note that all figures except for the ones in `configuration_name/figures/per_run` folder and barplots generated by `barplots_from_csvs` for all runs are created in the `main_analysis` function.
...
- **Configured Training Analysis Output**:
   ...
   - `configuration_name/figures` folder: Figures illustrating the resutls of training with the relevant configuration.  <details><summary>List of figures</summary>
      ...
      - **`c_f_kappa_f.png`, `c_f_kappa_f_train.png`**: Barplots of $c$, $f$ and $\kappa_f$ parameter values before and after training (stage 2) generated by `barplots_from_csvs` function. `c_f_kappa_f_train` excludes run that did not meet the conditions during the run-exclusion procedure.
      - **`J_pre_post.png`, `J_pre_post_train.png`**: Barplots of $J$ parameter values before and after training (stage 2) generated by `barplots_from_csvs` function. `J_pre_post_train` excludes run that did not meet the conditions during the run-exclusion procedure.
      - **`kappas_Jmidsup.png`, `kappas_Jmidsup_train.png`**: Barplots of $\kappa$ parameter values before and after training (stage 2) generated by `barplots_from_csvs` function. `kappas_Jmidsup_train` excludes run that did not meet the conditions during the run-exclusion procedure.
      ...
      - **`offset_pre_post.png`, `offset_pre_post_train.png`**: Barplots of staircase and psychometric offset thresholds before and after training (stage 2) generated by `barplots_from_csvs` function. `offset_pre_post_train` excludes run that did not meet the conditions during the run-exclusion procedure.
      - **`r_pre_post.png`, `r_pre_post_train.png`**: Barplots of mean and max responses before and after training (stage 2) generated by `barplots_from_csvs` function.  `r_pre_post_train` excludes run that did not meet the conditions during the run-exclusion procedure.
      ...
      - `configuration_name/figures/per_run` folder: Figures showing the time-evolution of parameters per random initialization generated by `plot_results_from_csvs` function.
      </details>
