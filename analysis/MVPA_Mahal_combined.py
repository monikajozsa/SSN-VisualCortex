...
def MVPA_Mahal_analysis(folder,num_training, num_stage_inds=2, r_noise = True, sigma_filter=1, plot_flag=False):
    # Shared parameters
    ...
    for run_ind in range(num_training):
        ...
        r_mid_sup = filtered_model_response(folder, run_ind, ori_list= ori_list, num_noisy_trials = num_noisy_trials, num_stage_inds=num_stage_inds,r_noise=r_noise, sigma_filter = sigma_filter, plot_flag=plot_flag)
        ...
        for layer in range(num_layers):
            ...
            for stage_ind in range(num_stage_inds): 
                ...
                MVPA_scores[run_ind,layer,stage_ind, 0] = clf.fit(X_train, y_train).score(X_test, y_test)
                ...
                MVPA_scores[run_ind,layer,stage_ind, 1] = np.mean(np.array(scores))#clf.fit(X_train, y_train).score(X_test, y_test)
...
def MVPA_Mahal_from_csv(folder, num_training, num_stage_inds=2, sigma_filter=5, r_noise=True, plot_flag=False, recalc = True):
    ...
    if recalc:
        MVPA_scores, df_mahal, df_LMI, mahal_train_control, mahal_untrain_control, mahal_within_train, mahal_within_untrain = MVPA_Mahal_analysis(folder,num_training, num_stage_inds, r_noise = r_noise, sigma_filter=sigma_filter, plot_flag=plot_flag)
