...
def smooth_data(X, gridsize_Nx =9, sigma = 1):
    ...
    num_phases = X.shape[0]//num_grid_points
    smoothed_data= np.zeros((num_grid_points,num_phases))
    for phase in range(num_phases):
        trial_response = X[phase*num_grid_points:(phase+1)*num_grid_points]
        ...
        smoothed_data=smoothed_data.at[:, phase].set(smoothed_data_temp) 
...
def filtered_model_response_task(file_name, untrained_pars, ori_list= np.asarray([55, 125, 0]), num_noisy_trials = 100, num_SGD_inds = 2, r_noise=None, sigma_filter = 1, gridsize_Nx=9):
    ...
    if num_SGD_inds==3:        
        if numpy.min(df['stage'])==0:
            ...
            print('Warning: No pretraining data but filtered_model_response was asked to be calculated for pretraining!')
...
def MVPA_score(folder, num_training, num_SGD_inds=2, sigma_filter=5):
    ...
    for run_ind in range(num_training):
        ...
        response_all, SGD_step_inds = filtered_model_response_task(file_name, untrained_pars, ori_list= ori_list, num_noisy_trials = num_noisy_trials, num_SGD_inds=num_SGD_inds, r_noise=True, sigma_filter=5)
        ...
        for layer in range(num_layers):
            for SGD_ind in range(num_SGD_inds):
                ...
                for ori_ind in range(len(ori_list)):
                    ...
                    MVPA_scores[run_ind,layer,SGD_ind, ori_ind] = clf.fit(X_train, y_train).score(X_test, y_test) # this model gave me very bad accuracies, so I used logistic regression instead
                
    return MVPA_scores, LogReg_scores


def MVPA_score_from_csv(final_folder_path, num_training, folder_to_save, file_name='MVPA_scores', num_SGD_inds=2, sigma_filter=5):
    ''' Calculate MVPA scores for before pretraining, after pretraining and after training - score should increase for trained ori more than for other two oris especially in superficial layer'''
    plt.close()
    MVPA_scores, LogReg_scores = MVPA_score(final_folder_path,num_training, num_SGD_inds, sigma_filter=sigma_filter)
    ...
    print('Before and after for 55, sup layer:',[np.mean(MVPA_scores[:,0,0,0]),np.mean(MVPA_scores[:,0,-1,0])])
    print('Before and after for 55, mid layer:',[np.mean(MVPA_scores[:,1,0,0]),np.mean(MVPA_scores[:,1,-1,0])])
    print('Before and after for 125, sup layer:',[np.mean(MVPA_scores[:,0,0,1]),np.mean(MVPA_scores[:,0,-1,1])])
    print('Before and after for 125, mid layer:',[np.mean(MVPA_scores[:,1,0,1]),np.mean(MVPA_scores[:,1,-1,1])])
    print('Before and after for 0, sup layer:',[np.mean(MVPA_scores[:,0,0,2]),np.mean(MVPA_scores[:,0,-1,2])])
    print('Before and after for 0, mid layer:',[np.mean(MVPA_scores[:,1,0,2]),np.mean(MVPA_scores[:,1,-1,2])])
